...
public final class DwellingDensity
{
	 ...
	 * @return
	 */
	public static SimpleFeatureCollection averageDensity( SimpleFeatureSource populationSource,
	...
	public static SimpleFeature averageDensity( SimpleFeatureSource dwellingSource, SimpleFeature roi,
			String densityAttribute )
	{
		try
		{
			SimpleFeatureIterator subRegions = featuresInRegion( dwellingSource, (Geometry) roi.getDefaultGeometry() )
					.features();
			...
			throws IOException
	{
		...
		LOGGER.debug( "Running intersects filter of {} and {}, features: {}", featureSource.getBounds(),
				roi.getEnvelope(), features.size() );
		return features;
	}
}
