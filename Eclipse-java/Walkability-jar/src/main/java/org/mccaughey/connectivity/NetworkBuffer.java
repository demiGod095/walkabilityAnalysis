...
public final class NetworkBuffer
{
			...
			Double bufferDistance ) throws IOException
	{
		...
		LOGGER.debug( "Finding service area for point {}. Network distance: {}, Buffer distance {}",
				new Object[] { pointFeature.getID(), networkDistance, bufferDistance } );
		...
		if( nearestLine == null )
		{
			LOGGER.error( "Failed to snap point {},{} to network", pointFeature.getID(),
					pointFeature.getDefaultGeometryProperty().toString() );
			...
			SimpleFeatureCollection networkRegion, Point pointOfInterest )
	{
		...
		Geometry lineB = connectedLine.extractLine( connectedLine.project( minDistPoint ),
	...
	private static FeatureGraphGenerator buildFeatureNetwork( SimpleFeatureCollection featureCollection )
	{
		...
		SimpleFeatureType edgeType = createEdgeFeatureType(
			...
			String id )
	{
		...
		while( edges.size() > 0 )
		{
			...
			Set<Edge> unjoined = new HashSet<Edge>();
			for( Edge edge : edges )
			{
