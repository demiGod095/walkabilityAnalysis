...
public class ConnectivityIndexFJ extends RecursiveAction
{
			...
			SimpleFeatureCollection regionsFeatureCollection )
	{
		...
		results = new DefaultFeatureCollection();
	}
	...
	{
		if( regions.size() <= 100 )
		{ // if there is only one region compute the
			...
			try
			{
				...
				while( regionsIter.hasNext() )
				{
					...
					SimpleFeature connectivityFeature = ConnectivityIndex.connectivity( roadsFeatureSource, region );
		...
		{ // otherwise split the regions into single region arrays and
			// invokeAll
			...
			while( features.hasNext() )
			{
				...
				if( featureCount == 100 )
				{
					ConnectivityIndexFJ cifj = new ConnectivityIndexFJ( roadsFeatureSource, regionsSubCollection );
					indexers.add( cifj );
			...
			for( ConnectivityIndexFJ ci : indexers )
			{
				...
				{
					completeExceptionally( ci.getException() );
				}
	...
	public void connectivity()
	{
		...
		try
		{
			...
			{
				LOGGER.error( "ForkJoin connectivity calculation failed: {}", getException().toString() );
				completeExceptionally( getException() );
			}
		}
		finally
		{
